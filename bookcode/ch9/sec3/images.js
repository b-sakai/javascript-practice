...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Answer available immediately: Promise.resolve
  {
    const loadImage = url => {
    ...
    loadImage().then(img => { imgdiv.appendChild(img) })
    loadImage('../../hanafuda/1-2.png').then(img => { imgdiv.appendChild(img) })
  }

  // Answer fails immediately: Promise.reject
  {
    const loadImage = url => {
      if (url === undefined) {
    ...
    loadImage()
      .then(img => { imgdiv.appendChild(img) },
        reason => { imgdiv.appendChild(brokenImage()) } )
